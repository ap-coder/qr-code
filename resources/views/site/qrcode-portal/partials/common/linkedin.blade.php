<div class="row col-md-12 mb-10 channels-container linkedin-container" random="{{ $rand ?? 0 }}">
    
    <div class="col-sm-12 col-md-4 box-label">
        <span>LinkedIn</span>:
        <span class="ml-10">
            <div class="channel-bgd-linkedin">
                <i class="fab fa-linkedin"></i>
            </div>
        </span>
    </div>

    <div class="col-sm-12 col-md-8 channel-input-container">
        <input type="hidden" name="icon_class[]" value="fab fa-linkedin">
        <input type="hidden" name="social_name[]" value="LinkedIn">
        <div class="input-group">
            <span class="input-group-addon" id="prefix-addon13">
                URL *
            </span>
            <input type="url" class="form-control" placeholder="www.linkedin.com/company/name" required="required"
                id="channelInput-13" name="url[]">
        </div>
        <div class="input-group">
            <span class="input-group-addon" id="label-addon13">Text</span>
            <input type="text" class="form-control channelText" placeholder="Join my network" id="qr_media_channels_channels_13" name="channel_label[]">
        </div>
    </div>
    @include('site.qrcode-portal.partials.common.right-icons')
</div>