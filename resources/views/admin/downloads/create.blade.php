...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.download.title_singular') }}
    </div>
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.downloads.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('frame') ? 'is-invalid' : '' }}" name="frame" id="frame">
                    ...
                    @foreach(App\Models\Download::FRAME_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('frame', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('frame'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('frame_color') ? 'is-invalid' : '' }}" type="text" name="frame_color" id="frame_color" value="{{ old('frame_color', '#000000') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('frame_text') ? 'is-invalid' : '' }}" type="text" name="frame_text" id="frame_text" value="{{ old('frame_text', 'SCAN ME') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('code') ? 'is-invalid' : '' }}" name="code" id="code">
                    ...
                    @foreach(App\Models\Download::CODE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('code', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('code'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('qr_color') ? 'is-invalid' : '' }}" name="qr_color_id" id="qr_color_id">
                    @foreach($qr_colors as $id => $entry)
                        <option value="{{ $id }}" {{ old('qr_color_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
