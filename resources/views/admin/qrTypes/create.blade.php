...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.qrType.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.qr-types.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('industries') ? 'is-invalid' : '' }}" name="industries[]" id="industries" multiple>
                    @foreach($industries as $id => $industry)
                        <option value="{{ $id }}" {{ in_array($id, old('industries', [])) ? 'selected' : '' }}>{{ $industry }}</option>
                    @endforeach
                </select>
